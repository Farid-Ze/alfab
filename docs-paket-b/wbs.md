...
- **2.4 Integration: Legacy ERP**
    ...
    - **Stock Sync Status API:** Expose `last_sync_at` untuk delay banner. (0.25 MD)
    - **Weekly Reconciliation Job:** Scheduled diff + notification on >5% discrepancy. (0.75 MD)
- **2.5 Integration: WhatsApp & Routing**
    ...
    - **Credit Warning in WA Message:** Include AMBER warning in WA deep link text. (0.25 MD)
- **2.6 Invoice Module**
...
- **2.8 Order Enhancements**
    - **PENDING_SYNC Flag:** Set flag when ERP down, sync on recovery. (0.25 MD)
    - **Email Fallback for Checkout Notification:** Send email if WA fails. (0.25 MD)


### EPICS 3: FRONTEND EXPERIENCE (Total: 42.5 MD)
...
- **3.8 Admin SLA Dashboard:** Order routing monitor, SLA status, escalation view. *SubDist tidak memiliki dashboard.* (1 MD)
- **3.9 Resilience UI Components**
    - **Stock Sync Delay Banner:** Warning banner jika sync > 30 menit. (0.5 MD)
    - **PENDING_SYNC Badge:** Badge pada order history. (0.25 MD)
    - **Savings Visualization:** Harga Partner vs Harga Silver (baseline). (0.75 MD)
