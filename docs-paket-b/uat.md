...
| ID | Skenario Pengujian | Langkah Pengujian (Steps) | Ekspektasi Hasil (Expected Result) | Status |
| --- | --- | --- | --- | --- |
| **UAT-20** | **Geo-Routing ke Sub-Distributor** | 1. Login Partner Surabaya  2. Checkout order | Order di-route ke WA Sub-Distributor **Surabaya** (bukan Jakarta). Cek `routed_to_subdist_id`. WA Deep Link terbuka. | $$$$ |
| **UAT-21** | **Admin Monitor Routed Orders** | 1. Login Admin  2. Buka SLA Dashboard | Semua order yang sudah di-route ke SubDist tampil dengan status dan waktu tunggu. | $$$$ |
| **UAT-22** | **Manual Status Update by Admin** | 1. Admin pilih order 2. Update status ke PROCESSING | Status order berubah jadi "PROCESSING". Record tercatat di audit log. | $$$$ |
