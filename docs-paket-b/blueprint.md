...
- **Algoritma Safety Buffer (Per SKU):**
    1. **Fast Moving SKU (Shampoo/Oxidant):**
        - *Definisi:* Produk dengan frekuensi penjualan > 50 transaksi/bulan (dihitung dari 30-day rolling window).
        ...
        - *Data Source:* Histori penjualan dari ERP atau ORDER_ITEMS table.
...
> [!NOTE]
> **Sumber Data Credit Limit (Plafon Kredit):**
> - `credit_limit` Partner disimpan di tabel PARTNERS dan di-sync dari ERP saat initial data migration.
> - Perubahan credit limit dilakukan oleh Admin via Admin Panel atau melalui sync ERP (configurable).
> - Jika ERP tidak menyediakan credit limit API, nilai dikelola manual di sistem.
...
- **Tampilan:**
    - *"Harga Pasar: Rp 1.500.000"* (menggunakan **TIER_SILVER** price sebagai baseline)
    - *"Harga Spesial Partner: Rp 1.350.000"*
    - **"ANDA HEMAT: Rp 150.000 (Benefit Gold)"**
- *Dampak:* Salon merasa sudah mendapatkan "Best Price" sehingga sungkan untuk menawar lagi.

> [!NOTE]
> **Implementasi Teknis:** `savings = silver_price_total - partner_tier_price_total`. Tier Silver berfungsi sebagai "harga pasar" karena merupakan tier terendah (tanpa benefit).
...
## 7. KONFIGURASI AGILE (ADMIN PANEL)
...
| `loyalty.point_rate` | 10000 | IDR per 1 poin (Rp 10.000 = 1 poin) |
...
| `sla.reminder_hours` | 18 | Jam sebelum kirim reminder ke SubDist |
| `sla.escalation_hours` | 24 | Jam sebelum eskalasi ke HQ |
