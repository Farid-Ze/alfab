# AS-BUILT SPECIFICATION (REALISASI)
>
> Dokumen ini adalah spesifikasi final ("Realization") dari sistem yang dibangun, berbasis pada Proposal awal.
...
2) Daftar brand + deskripsi singkat per brand

### C) Katalog produk (minimum)

1) Kategori produk
2) Data minimum per produk: nama, deskripsi singkat, foto

### D) Education/Events

1) Data minimal event/article: judul, tanggal (jika event), ringkasan, CTA/link

### E) Partnership / Become Partner

1) Kriteria partner
2) Field form yang disepakati

### F) WhatsApp CTA

1) Nomor WhatsApp: **081511688745**
2) Pesan prefill

### G) Domain & koordinasi go-live

1) Domain publik: `alfabeautycosmetica.com`
2) Domain redirect/defensive: `alfabeautycosmetica.co` (redirect ke `.com`)
3) PIC IT yang memegang akses DNS/SSL saat go-live
4) PIC IT yang memegang akses DNS untuk setup record email (**SPF/DKIM/DMARC**) pada domain pengirim notifikasi
...
1) Daftar istilah yang wajib dipertahankan
2) Preferensi gaya bahasa EN (formal B2B)
...
| Field | Value |
|---|---|
| Status | **DELIVERED / AS-BUILT** |
| Approved by | Bapak Edy |
| Approval date | 2026-01-21 |
| Approval method | Google Meet |
| Keterangan |  |
